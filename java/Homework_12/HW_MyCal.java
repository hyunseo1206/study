...
{	
	...
	public MyCal() {
		frame=new JFrame("2115411-최현서");
		menuBar=new JMenuBar();

		menu=new JMenu("보기(V)");
		menu.setFont(new Font("맑은 고딕", 0, 12));
		
		menu2=new JMenu("편집(E)");
		menu2.setFont(new Font("맑은 고딕", 0, 12));
		
		menu3=new JMenu("도움말(H)");
		menu3.setFont(new Font("맑은 고딕", 0, 12));
	...
	public void gui() {
		...
		textField.setEditable(false); 						  // 텍스트필드창에 텍스트를 작성할 수 없게 함.
		
		textfieldNorth.setHorizontalAlignment(JTextField.RIGHT);  // 우측정렬
		textfieldNorth.setEditable(false); 						  // 텍스트필드창에 텍스트를 작성할 수 없게 함.
		...
		for(int i=0; i<strGrid.length; i++){
			...
			Button.setBackground(new Color(241,244,249));   // 버튼 집어넣기
		...
		frame.setSize(340,420);                                   	//frame 의 크기  
